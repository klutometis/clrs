...
(define (fastest-way/compact a t e0 e1 x0 x1 n)
  (let ((f0 (+ e0 (array-ref a 0 0)))
        (f1 (+ e1 (array-ref a 1 0)))
        (l (make-array '#(#f) 2 n))
        (f* 0)
        (l* 0))
    (loop ((for j (up-from 1 (to n))))
          (let ((first-straight (+ f0
                                   (array-ref a 0 j)))
                (first-cross (+ f1
                                (array-ref t 1 (- j 1))
                                (array-ref a 0 j)))
                ...
                (second-cross (+ f0
                                 ...
                                 (array-ref a 1 j))))
            (if (<= first-straight first-cross)
                (begin (set! f0 first-straight)
                       (array-set! l 0 0 j))
                (begin (set! f0 first-cross)
                       (array-set! l 1 0 j)))
            (if (<= second-straight second-cross)
                (begin (set! f1 second-straight)
                       (array-set! l 1 1 j))
                (begin (set! f1 second-cross)
                       (array-set! l 0 1 j)))))
    (let ((first-finish (+ f0 x0))
          (second-finish (+ f1 x1)))
      (if (<= first-finish second-finish)
          (begin (set! f* first-finish)
                 (set! l* 0))
