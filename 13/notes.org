# These options are self-contradictory; num:nil wins:
...
** Exercises
*** 13.1-1
    See [[fig:13.1-1a][13.1-1a]], [[fig:13.1-1b][13.1-1b]], [[fig:13.1-1c][13.1-1c]].
#+CAPTION: BST on ${1, 2, ..., 15}$ with black-height 2
#+LABEL: fig:13.1-1a
#+ATTR_LaTeX: width=\textwidth
    [[./13.1-1a.png]]
#+CAPTION: BST on ${1, 2, ..., 15}$ with black-height 3
#+LABEL: fig:13.1-1b
#+ATTR_LaTeX: width=\textwidth
    [[./13.1-1b.png]]
#+CAPTION: BST on ${1, 2, ..., 15}$ with black-height 4
#+LABEL: fig:13.1-1c
#+ATTR_LaTeX: width=\textwidth
    [[./13.1-1c.png]]
*** 13.1-2
    If 36 is colored black, it is no longer a red-black tree; since
    uniform black-height is violated. 36 may be colored red, however,
    since it has two nil children that are black.

    See [[fig:13.1-2][13.1-2]].
#+CAPTION: \texttt{tree-insert} with 36
#+LABEL: fig:13.1-2
#+ATTR_LaTeX: width=\textwidth
    [[./13.1-2.png]]
*** 13.1-3
    Yes; the black height is still uniform, and there are no
    constraints on a black node as regards its children.
*** 13.1-4
    2 if both children are black; 3 if one of them is red (and the
    other black); 4 if both the children are red. The resultant height
    of the tree is its black height: all depths are the same.
*** 13.1-5
    Longest: every other node is black; shortest: every node
    black. Same black height: 2x.
*** 13.1-6
...
*** 13.1-7
    ...
    $\sum_{n = 1}^{h(T)/2} 2^{2n - 1} : \sum_{n = 1}^{h(T)/2} 2^{2n}$
* 13.2
...
*** DONE 13.3-2
    CLOSED: [2010-09-04 Sat 04:37]
    - CLOSING NOTE [2010-09-04 Sat 04:37] \\
      we had a spurious 21.
    the result of 13.3-2.scm contradicts my hand-drawn tree; which is
    correct? partially because you have a spurious 21, dumbass! you
    meant 12, i believe.
...
#+LABEL: fig:13.3-2
    [[./13.3-2.png]]
#+INCLUDE: "13.3-2.scm" src scheme
*** 13.3-3
    #+BEGIN_QUOTE
    We call the number of black nodes on any path from, but not
    including, a node $x$ down to a leaf the **black-height** of the
    node.
    #+END_QUOTE

    Black height is exclusive, therefore; [[property 5]] states that "all
    paths from the node to descendant leaves contain the same number
    of black nodes."

    See [[fig:13.3-3a][13.3-3a]], [[fig:13.3-3b][13.3-3b]], [[fig:13.3-3c][13.3-3c]].

#+CAPTION: Case 1 of \texttt{rb-insert} maintains black-height when $z$ is a right child.
#+LABEL: fig:13.3-3a
#+ATTR_LaTeX: width=\textwidth
    [[./13.3-3a.png]]
#+CAPTION: Case 1 of \texttt{rb-insert} maintains black-height when $z$ is a left child.
#+LABEL: fig:13.3-3b
#+ATTR_LaTeX: width=\textwidth
    [[./13.3-3b.png]]
#+CAPTION: Cases 2 and 3 of \texttt{rb-insert} maintain black-height.
#+LABEL: fig:13.3-3a
#+ATTR_LaTeX: width=\textwidth
    [[./13.3-3c.png]]
*** 13.3-4
    =rb-insert-fixup= only ever reddens the grandparent of $z$ in
    cases 1 and 3; thus, $z$ would have to be the child of $root[T]$
    to redden $nil[T]$ (since $nil[T]$ is only parent to
    $root[T]$). Furthermore, $z$'s parent would have to be red to
    redden its grandparent (otherwise, the =while= loop in line 1
    would never be executed); but $root[T]$ is always
    black. Therefore, since "child of a red parent" and "child of
    $root[T]$" are contradictory; $nil[T]$ is never reddened.

    Case 2, which sets $z = p[z]$ before rotating, preserves
    $p[p[z]]$; and is therefore indistinguishable from case 3.
*** 13.3-5
    $z$, upon insertion, is red; $z$ is only ever blackened if it
    becomes the root (in which case $n = 1$) and remains, otherwise,
    red.

    If $z$'s parent is black, at least one node (namely $z$) remains
    red. If $z$'s parent is red, case 1 blackens the parent and uncle
    and reddens the grandparent; leaving at least two nodes (namely
    $z$ and $p[p[z]]$) red. Case 2 leaves at least two nodes red ($z$
    and $p[z]$) on its way to transforming to case three; and case 3
    performs a rotation but does not alter the number of black or red
    nodes, leaving $z$ and $z$'s sibling red.

    In all cases, therefore, at least one red node is preserved when
    $z > 1$.
*** 13.3-6
    [[http://en.wikipedia.org/wiki/Red-black_tree][This]] is interesting, by the way:
    #+BEGIN_QUOTE
    ...
    structures, used to construct associative arrays and sets which
    can retain previous versions after mutations. The persistent
    version of red-black trees requires O(log n) space for each
    insertion or deletion, in addition to time.
    #+END_QUOTE
    Save for problem 13-1.

    When searching for an appropriate leaf to insert $z$, construct a
    stack of ancestors; pass the stack to =rb-insert-fixup=.

#+INCLUDE: "13.3-6.scm" src scheme
* 13.4
  - deletion takes $O(\lg n)$
  - splice out node; call fixup.
    - bst-splice: no children: parent \to =nil=; single child: splice;
      two children: replace with successor
  - differences from bst-splice
    - =nil= replaced by =nil=-sentinel;
    - $p[x] \gets p[y]$ performed unconditionally (setting the
      sentinel once in a while);
    - =rb-delete-fixup= if $y$ is black (if $y$ is red: black-height
      hasn't changed; no red nodes have become adjacent; root remains
      black).
  #+BEGIN_SRC org
    ,- rb-delete-fixup(T, x)
    ...
    ,          - w = right[p[x]]
    ,          - if color[w] = red
    ,            # case 1: x's sibling is red. switch the colors of w and
    ,            p[x] and then perform a left-rotation on p[x]; convert
    ,            case 1 into case 2.
    ,            - then
    ,              - color[w] = black
    ,              - color[p[x]] = red
    ...
    ,            # case 2: x's sibling w is black; both of w's children
    ,            are black. take one black off x and w; repeat the while
    ,            loop with p[x], adding an extra black to p[x]. if we
    ,            entered through case 1, x is red and the loop terminates.
    ,            - then
    ,              - color[w] = red
    ,              - x = p[x]
    ,            - else
    ,              - if color[right[w]] = black
    ,                # case 3: w is black, left[w] is red, 
    ,                right[w] is black. switch the colors of w and left[w],
    ,                right-rotate w; transforming case 3 into case 4.
    ,                - then
    ,                  - color[left[w]] = black
    ,                  - color[w] = red
    ,                  - right-rotate(T, w)
    ,                  - w = right[p[x]]
    ,                    # case 4: w is black, right[w] is red. remove the
    ,                    extra black on x; setting x to be the root causes
    ...
    ,  - color[x] = black
  #+END_SRC
    - "in other words, the extra black on a node is reflected in x's
      pointing to the node rather than in the color attribute:" what a
      bizarre and awkward notion!
** Exercises
*** 13.4-1
    The =while= loop in =rb-delete-fixup= ascends the tree until it
    finds the root or a red node. If it finds the root, it will color
    it black; regardless of whether the root had been reddened by
    splicing from =rb-delete= or rotation by case 4.

    If the =while= loop encounters a red node before it reaches the
    root, the root will have been unmodifiedly black; since
    =rb-delete-fixup= only proceeds root-wise from the highest
    modified node in =rb-delete=.
*** 13.4-2
    If $x$ is red, then $y$ (i.e. $x$'s parent, and $z$ or $z$'s
    successor) has at least one non-=nil= child ($x$) and is not red
    (otherwise, $x$ would be black); therefore =rb-delete-fixup= is
    called on $x$. $y$ also has a black sibling.

    $y$ ever has only one child $x$: $x$ is either a left or right
    child if $y$ is the self-same as $z$ (in which case, $z$ has only
    one child); or $x$ is a right child if $y$ is $z$'s successor (in
    which case, $z$ has two children). The cases are symmetrical;
    let's take the case in which $x$ is $left[y]$.

    The result, depending on whether $x$'s sibling $w$ has two black
    children or not, is formerly case 2; and, latterly, a case 3 ->
    case 4 transformation. Both of which result in a restoration of
    property 4.
   ...
   #+BEGIN_SRC scheme
     ...
     (let* ((root nil)
       ...
       (test
        ...
        (tree->pre-order-key-color-list root)))
   #+END_SRC
** DONE parentless-insert
   CLOSED: [2010-09-22 Wed 16:26]
   - CLOSING NOTE [2010-09-22 Wed 16:26] \\
     whew; this was a tough one, actually: had to apprehend exactly what
     was going on.
   #+BEGIN_SRC scheme
     (load "red-black-tree.scm")
